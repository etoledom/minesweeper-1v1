...
impl Server {
    ...
    fn request_identification(&self, addr: SocketAddr) {
        let message = Message::Text(SimpleMessage::new("identify").to_json());
    ...
    fn handle_received_message(&self, msg: Message, addr: SocketAddr) {
        ...
        if let Ok(message) = IdentificationMessage::from_json(message_string) {
            ...
            self.send_message_to_addr(&addr, SimpleMessage::new("waiting_enemy").to_json());
    ...
    fn send_open_games(&self, addr: SocketAddr) {
        ...
        sender.unbounded_send(Message::Text(message.to_json())).unwrap();
    }
